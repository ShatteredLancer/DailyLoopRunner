...
Trade Scheduler 的详细边界、阶段状态和真实页面验证顺序见 [docs/TRADE_SCHEDULER_DESIGN_ZH.md](docs/TRADE_SCHEDULER_DESIGN_ZH.md)。挂牌只能由 EA Trade Adapter 的单一 `services.Item.list()` 调用点执行，且必须经过 Prepared Plan、一次性 token、显式确认、item ID 重解析、价格限制二次刷新和逐项回执。当前手动门禁只允许同一来源的 1-2 张 Club 卡使用 `LIST N`，或 1-2 张 Transfer `inactive` 卡使用 `REPRICE N`；Transfer reprice 必须在 Prepare 和每次 mutation 前刷新 Transfer、复核同一 item ID/definition ID/pile，挂牌后按同一 item 和精确价格回读 Active。混合来源、超过两张、周期 reprice 和 `relistExpiredAuctions()` 仍不得开放。Listing diagnostics 不得导出确认 token、原始 EA runtime 对象或原始错误 response。
Scheduled Transfer reprice 只能接受 Transfer-only、`once`、`expiredPolicy=reprice`、`maxListings=1..2` Job，并要求未来 15 秒至 15 分钟内的运行时间和精确 `RUN REPRICE ONCE N` 人工确认。执行器必须在 Prepare 前自动 relock，按 Prepared 实际数量使用 `REPRICE N` transaction、逐张 heartbeat、同一 item 身份与价格回读，并在结束后保持 Scheduler 暂停、live disabled、Job disarmed；混合来源、超过两张、周期 reprice、批量 relist 和 `relistExpiredAuctions()` 仍必须 fail-closed。
...
Trade Scheduler Job Store、时区/misfire 计算、lease、Coordinator 和 UI 可以在自动执行关闭时开发和测试；Scheduler 必须默认 paused、`liveExecutionEnabled=false`，导入 Job 必须解除 armed。当前自动门禁只允许恰好一个 `once` Job：同一来源最多两张 Listing/reprice，或 Rare Gold、最多两个相邻评分且 `quantity<=2` 的 Buy；启动时必须立即回锁。Recurring schedule、混合 Listing 来源、第三张卡、其它 Buy card class 和更高价格/预算仍不得开放。单元/Fake 测试不能替代新的独立门禁和有序实机验证。

TS2c/TS3 的单卡约束是历史验证阶段；当前门禁在保留相同回锁和确认机制的前提下允许 `maxListings=1..2`。必须恰好一个 enabled+armed 的 `once` Listing Job，来源严格为 Club 或 Transfer 单一来源、runAt 在未来 15 秒至 15 分钟内、misfire 仅允许 skip 或不超过 15 分钟的 grace-window，并由用户输入模式对应的 `RUN ONCE N` 或 `RUN REPRICE ONCE N` 显式开启。Job 启动前必须立即设置 paused、关闭 `liveExecutionEnabled` 并解除 armed；任务错过、过期 lease、circuit 变化、配置变化或 tick 异常同样必须回锁。该门禁不得扩展到 daily/interval/window、next-login、第二个 armed Job、混合来源或超过两张。
...
手动真实购买门禁要求 Job enabled、unarmed、manual、Rare Gold、最多两个相邻评分、`quantity=1..2`，单卡价格不得超过 2000，总预算不得超过 4000，Scheduler 必须保持 paused 且 `liveExecutionEnabled=false`。用户必须输入精确的 `BUY N MAX <price>`，执行前重新生成只读 Preview，并在同一 Operation Coordinator 与跨 Tab lease 下运行；每次确认最多调用 N 次 `buyNowItem()`，每次 Buy 前必须 heartbeat lease。竞争失败会消耗一次 mutation attempt；lease 丢失、circuit 变化或任何不明确结果都必须停止整个 Run，保留之前的部分成功，且不得以同一确认重试。该门禁不得扩展到第三个评分、非相邻评分、数量大于 2、Common/Special、单卡超过 2000 或总预算超过 4000。
...
定时 Buy 只允许恰好一个 enabled+armed 的 `once` Buy Job，Rare Gold、最多两个相邻评分、`quantity=1..2`，单卡价格不得超过 2000、总预算不得超过 4000，misfire 仅允许 skip 或不超过 15 分钟的 grace-window。激活前必须显式持久化全局 `minimumRetainedCoins`，不得提供静默默认值；Job 局部值只能提高有效底线，不能降低全局值，并要求精确确认 `RUN BUY ONCE N RESERVE <effective reserve>`。执行器必须在任何 Preview、搜索或 Buy 前通过 Job Store 单次 `relock()` 原子暂停、关闭 live execution 并解除全部 armed；Scheduler 继续拥有跨 Tab lease，Buy 执行器不得另取第二条 lease。每次计划授权最多 N 次 `buyNowItem()`，写 History 前必须走 Buy allowlist sanitizer。Recurring schedule、第三个评分、非相邻评分、其它 card class 和更高价格/预算仍必须 fail-closed。
...
所有 EA Trade 网络调用必须在 `ea/trade.js` 的唯一调用点取得共享请求预算许可；本地 capability/repository inspect 和第三方 Provider HTTP 不计入该 EA 预算。默认预算固定为跨标签页共享的 5 分钟 30 次滑动窗口，支持 Web Locks 时许可写入必须通过独占锁串行化。Listing 必须在 Prepare 前原子预留并让 Prepare/Transaction 共用同一 reservation，1-2 张均至少预留 12 次；Buy 按最坏对账路径动态预留，单张 14 次、双张 28 次。余额不足只能进入 `cooldown` 或在 Preview/Prepare/mutation 前阻断。预算耗尽不得记为 EA Circuit failure，也不得提供清空、提高上限或跳过许可的 UI/API。诊断只允许输出窗口、使用/剩余数量、动作聚合、恢复时间和锁支持状态，不得输出持久事件明细、卡片/definition、价格、URL 或原始响应。

Scheduler 公平选择状态必须持久化且保持有界，只记录最后实际 dispatch 的 Job ID/type/time 和累计次数。Buy 与 Listing 同时到期时优先选择与上次 dispatch 不同的类型，同类型内继续按最早 `nextRunAt` 和稳定 ID 排序；lease 未取得、misfire advance、waiting/cooldown 不得计为 dispatch。该公平核心不得绕过当前“恰好一个 enabled+armed Job”的 guarded 生产入口，也不得据此开放 recurring、超过两张、非相邻 rating range 或其它 Buy card class。

Listing 和 Buy live Run 都必须写入各自的持久 bounded Journal，保存 Run/Job ID、最多两个 item 的 Prepare、mutation boundary、脱敏 EA response、reconciliation 和 terminal phase。Journal 不得保存 token、Cookie、原始 EA 对象或 response body。浏览器在 mutation boundary 前终止时可以写 blocked History 后清理 expired lease；任一 item 已跨过 boundary 且结果不确定时，必须保留 Journal 和 Lease 证据、阻止新 Run，并要求人工核对，绝不得自动重试该 item。多 item Run 的第一张成功、第二张失败/不明确时必须保留部分成功回执并跳过剩余卡。
