...
SHA256: EB76503A6573E8FFA7C19A8E55E44D18775ABC552D5319C6DE488E01D2E3F865
```

该哈希是 `22-56-54` diagnostics 通过后的当前稳定基线，已在 FSU 与 Enhancer 同时启用时验证 exact scoped capture、最终数量和性能。上一份稳定回滚点为：

```text
backup-20260722-before-club-performance-phase2
```

更早的 `3CF01F8F...AA5CF` 保留为 FSU-P1 前的 `05-19-10` 成功基线，不再是当前发布候选。
...
- 全量 exact scoped 扫描仍存在，只是改为后台执行；当前 Enhancer 开启的稳定复验约 52.072 秒。后续优化不能以牺牲实体级正确性为代价。
