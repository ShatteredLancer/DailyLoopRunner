...
- `DailyLoopRunner.user.js`: `0.2.85`
...
`0.2.84` 起普通金卡材料不再受 FSU 的 `Only Untradeable`、`Exclude designated league`、`Exclude Evolution` 或 rarity-player 开关限制，避免便宜金卡因通用保护被排除。其它类型材料不足且确实受到 FSU 过滤时，日志会汇总生效的 FSU guard，并明确提示 Runner 没有绕过这些设置。

`0.2.85` 起普通金卡按“除非手动 Lock，否则都可做 SBC”处理：允许可交易普通金卡，Runner 普通金卡评分上限放宽到 `99`，84+ TOTW 前置的 rare gold requirement 也放宽到 `82-99`。FSU Lock player 始终保留；特殊卡仍受类型、数量、交易状态和 `maxSubmittedRating` 保护。FSU 自身建议关闭 `Only Untradeable`、`Exclude designated league` 和金卡区间优先，并把 Golden Player Range 设为 `99`。

Inventory-first SBC 会在选人阶段按 `definitionId` 去重，避免同一球员的两张重复卡被选入同一套 SBC 后保存时只剩 10/11 人；同时会继承 loop 级 `blockTradeable`。`0.2.85` 起该硬保护不再拦截普通金卡，但仍保护可交易特殊卡和其它非普通金材料。
...
- `84x10 MVP (1 run)`
  ...
  - 若保护检查通过但 submit 仍不亮，会最多尝试 8 次用普通金 `<=99` 补分并重新保存检查；仍不亮则停止。唯一 requirement special 仍限制为 `<=88`。
  - 使用 FSU/Enhancer 填阵容，但提交前由脚本检查 squad。
  - 默认 `maxSubmittedRating: 88`、`maxNormalGoldSubmittedRating: 99`、`requiredSpecialCount: 1`、`requiredSpecialKind: totw-tots-fof`、`requiredSpecialMinRating: 84`、`allowedSpecialCount: 1`、`blockTradeable: true`。`blockTradeable` 只继续保护特殊卡/非普通金材料。
  - 自动 TOTW 前置 SBC 默认 `maxSubmittedRating: 88`、`maxNormalGoldSubmittedRating: 99`、`blockSpecial: true`、`blockTradeable: true`，rare gold 材料允许 82-99，并优先从低分材料开始；可交易普通金卡也可用于补分。
...
- `maxNormalGoldSubmittedRating`: 可选，仅控制普通金卡的最高 rating；特殊卡仍使用 `maxSubmittedRating`。84x10 和自动 TOTW 前置默认普通金卡到 `99`，requirement special 仍到 `88`。
...
- `blockTradeable`: 可选，`fillAndVerifySbc` 是否阻止可交易卡。`0.2.85` 起普通金卡是例外，仍可提交；该开关继续保护可交易特殊卡和其它材料。
