...
- `DailyLoopRunner.user.js`: `0.2.65`
...
Inventory-first SBC 会在选人阶段按 `definitionId` 去重，避免同一球员的两张重复卡被选入同一套 SBC 后保存时只剩 10/11 人。
...
选卡不够时，日志会追加 diagnostics：每个 pile 的总数、匹配数量、unique definition 数、duplicate signal 解析数量，以及主要过滤原因。若 FSU 锁卡被识别，`FSU settings sync` 会显示 `locked:n`，被排除的卡会计入 `fsu-locked-player`。
