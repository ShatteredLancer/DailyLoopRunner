...
进入 EA FC Web App 后，等待页面、FSU 和 Enhancer 初始化。面板出现 `Ready v0.5.57` 后即可开始；优化版 FSU 命中快速缓存时会进入 `trusted-provisional`，后台继续校验已恢复的 Club 缓存。Runner 会在每次保存 SBC 前只向 EA 校验本次选中的 Club 球员，全量校验结束后自动切换为普通 ready 状态。
...
Player Pick 与 Batch Open 使用统一逐卡 recap：每页最多 20 条，支持 Previous/Next、Preview 和 stopped/preserved/blocked 原因。普通卡按 Bronze、Silver、Common Gold、Rare Gold 85-、86-88、89+ 使用独立整行背景和评分色块；特殊卡优先使用 EA rarity card color map，无法安全读取时按 94-、95-97、98-99 三档回退。Pick 额外显示 Pick 序号和最终库存去向，Batch 保留包数量、跳过数量及金银铜汇总。
