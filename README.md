...
- `DailyLoopRunner.user.js`: `0.2.42`
...
84x10 测试推荐选择 `84x10 MVP (1 run)`。它会打开目标 SBC，让 FSU/Enhancer 填阵容，然后在提交前检查当前 squad。默认保护规则：只允许挑战要求的 1 张 TOTW，不提交额外特殊卡，不提交可交易卡，不提交 89+。如果库存里没有符合保护规则的 TOTW，live run 会先做 1 次 `84+ TOTW Upgrade` 并自动开包拿到 TOTW；`Dry run` 只会提示 live run 将执行这个前置步骤，不会提交 SBC。84x10 奖励包默认保留不打开。
