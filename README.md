...
进入 EA FC Web App 后，等待页面、FSU 和 Enhancer 初始化。面板出现 `Ready v0.5.32` 后即可开始；如果 FSU 正在后台校验已恢复的 Club 缓存，Runner 会在每次保存 SBC 前只向 EA 校验本次选中的 Club 球员，全量校验结束后自动切换为普通 ready 状态。
...
- `5 of 10 82+ Players Pick`：暂时保留静态配置作为 Provision 前置引用；两个 Challenge，每阵 11 张普通普金，最终选 5 张。该活动使用 EA Set 的 `timesCompleted/repeats` 计算当前剩余次数并执行到耗尽，不显示也不读取 `rounds`；已有同类型 pending Pick 会先处理，但不会占用 EA 返回的剩余 SBC 次数。
...
`FOF Glory Hunters Exhaustion Loop` 使用 9 张 `<=81` 普通金 common 反复提交 `FOF Glory Hunters Crafting Upgrade`，材料不足即停；默认在全部提交结束后再一次性打开 `5x 80+ Rare Gold Players Pack` 奖励包，中途不逐包打开。
...
- Player Pick 扫描只读取当前 SBC Set/Challenge/奖励元数据，不会提交 SBC 或领取 Pick。启动后会自动扫描，也可在 Options 中点击 `Scan Picks` 重扫；完全支持且不与静态配置重复的 Pick 会作为当前会话 Loop 加入下拉列表。EA Set 状态报告 `completed` 但 Challenge 元数据仍完整时，扫描器保留一次性运行探测入口；实际调用失败会明确停止。奖励身份、候选数、选择数、人数或材料条件不完整，以及过期或不再支持的 Pick，不会生成可运行 Loop。83+/84+ 已通过动态 Dry Run/Live 并移除静态活动配置；82+ 因 Provision 引用及多 Challenge 尚未复验，仍保留静态回退。
