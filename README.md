...
- `DailyLoopRunner.user.js`: `0.2.49`
...
- `autoTotwUpgrade`: 可选，对象或 `false`。当要求 TOTW/TOTS/FOF 但库存没有可用 requirement special 时，live run 会先执行该 TOTW 前置 SBC；设为 `false` 可关闭自动补 TOTW。
