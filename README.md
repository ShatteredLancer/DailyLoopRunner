...
- `DailyLoopRunner.user.js`: `0.2.84`
...
Runner 会尽量读取并跟随 FSU 的 SBC ignore player configuration，例如 Only Untradeable、Exclude Evolution、排除指定联赛、storage 优先、普通/稀有优先级、Lock player 等；84x10 等 FSU 自动填阵容的 loop 仍会在提交前执行 Runner 自己的硬保护检查。`0.2.84` 起普通金卡材料不再套用 FSU 的通用过滤，但 FSU Lock player 始终保留。
...
`0.2.84` 起普通金卡材料不再受 FSU 的 `Only Untradeable`、`Exclude designated league`、`Exclude Evolution` 或 rarity-player 开关限制，避免便宜金卡因通用保护被排除。FSU Lock player、Runner 的评分上限、特殊卡类型/数量保护、概念卡检查及各 loop 自己的可交易硬保护不变。其它类型材料不足且确实受到 FSU 过滤时，日志会汇总生效的 FSU guard，并明确提示 Runner 没有绕过这些设置。
...
84x10 测试推荐选择 `84x10 MVP (1 run)`。它会打开目标 SBC，让 FSU/Enhancer 填阵容，然后在提交前检查当前 squad。默认保护规则：只允许挑战要求的 1 张 84+ TOTW/TOTS/FOF，不提交额外特殊卡，不提交可交易卡，不提交 89+。如果库存里没有符合保护规则的 84+ TOTW/TOTS/FOF，live run 会先做 1 次 `84+ TOTW Upgrade`、点击 `Claim Rewards` 并自动开包拿到 TOTW；如果 FSU 填满 11 人但放错特殊卡，live run 会优先保留 squad 里已有的合格 84+ TOTW/TOTS/FOF，或从库存里注入最低评分的合格 84-88 TOTW/TOTS/FOF，并把低于 84 的 requirement special、多余/违规特殊卡替换掉，再重新检查。若保护检查通过但 submit 仍不亮，live run 会最多尝试 8 次普通金补分修复，只用 `<=88` 且符合 FSU/Runner 保护的普通卡。`0.2.71` 起如果 EA/FSU 更新导致 FSU 只填了 requirement special 而剩余槽位为空，Runner 会用同一套安全候选池补齐缺失普通卡，再保存并重新检查。`0.2.73` 起 84x10/填阵容类 SBC 每轮会先清理 unassigned，且直接保存 squad 的 repair 候选只从 `storage` / `club` 取，避免 EA `saveChallenge 475`。`0.2.74` 起 84x10 不再自动点击 requirement 右侧的 `Add`，避免 EA/FSU 先选到最高分色卡。`0.2.75` 起不再把 FC25 旧脚本里的 group 23 当作 TOTW，避免 Endo 这类 FC26 非 TOTW 特卡被误判为合格材料。`0.2.76` 起自动 `84+ TOTW Upgrade` 的第一组材料允许 84-85 rare，避免小号只差 1 张 85 时直接停住；仍然不提交 86+、特殊卡或可交易卡。`0.2.77` 起如果 Claim Rewards 按钮没有被普通 DOM 点击识别到，会在奖励弹窗上下文尝试 `AltRight` 领取热键。`0.2.78` 起如果奖励包 service 刷新找不到包，会自动打开 Store/Packs 页面刷新可见 pack cache，再继续自动开包。`0.2.79` 起自动开出的 TOTW 奖励卡会先重新绑定到当前 storage/club 等可提交缓存对象，再用于 84x10 required special 注入，避免 recent 奖励对象直接保存导致 `saveChallenge 475`。`0.2.80` 起自动 TOTW 奖励包开出后会直接把返回的球员 item 按重复状态移动到 club/storage/transfer，并等待可提交缓存追上，避免日志已标记 TOTW 但 84x10 仍误判缺材料。`0.2.81` 起做完前置 TOTW 后会等待新打开的目标 84x10 控制器和阵容对象真正就绪，避免误用已提交并清空的旧 TOTW 阵容页而得到 `0/11`。MVP 通过后可选择 `84x10 Loop (max 7)` 跑满最多 7 次；live guard 会按最坏情况提示 7 次目标 SBC + 最多 7 次自动 `84+ TOTW Upgrade`。`Dry run` 只会提示 live run 将执行前置 TOTW 或保存哪几处修复，不会提交 SBC。84x10 奖励包默认保留不打开。
`0.2.82` 起，单独 `84+ TOTW Upgrade` Loop 和 84x10 的自动 TOTW 前置都允许使用评分不超过 88 的普通不可交易金卡补分；84x10 本体允许普通不可交易金卡补分到 89，但 requirement special 仍限制在 84-88。特殊卡数量/类型、可交易卡、FSU 锁卡和更高评分卡继续受保护。安全材料仍不足时会打印最终评分分布并正常停止，不再抛出泛化错误栈。
...
- `84x10 MVP (1 run)`
  ...
  - 若保护检查通过但 submit 仍不亮，会最多尝试 8 次用普通金 `<=89` 补分并重新保存检查；仍不亮则停止。唯一 requirement special 仍限制为 `<=88`。
  - 使用 FSU/Enhancer 填阵容，但提交前由脚本检查 squad。
  - 默认 `maxSubmittedRating: 88`、`requiredSpecialCount: 1`、`requiredSpecialKind: totw-tots-fof`、`requiredSpecialMinRating: 84`、`allowedSpecialCount: 1`、`blockTradeable: true`。
  - 自动 TOTW 前置 SBC 默认 `maxSubmittedRating: 88`、`blockSpecial: true`、`blockTradeable: true`，rare gold 材料允许 82-88，并优先从低分材料开始；如果阵容评分不够，安全补分可使用 85-88 普通不可交易金卡。
  - WK 第一次只跑 `Dry run`，确认日志没有 `BLOCK` 后再考虑 live run。

- `84x10 Loop (max 7)`
  - 逻辑同 `84x10 MVP (1 run)`，但 `maxCompletions: 7` 且 `allowMultipleCompletions: true`。
  - live run 会触发二次确认；日志会按最坏情况提示最多 14 次 SBC：7 次 `10x 84+ Upgrade`，以及每次缺特卡时最多 1 次自动 `84+ TOTW Upgrade`。
...
- `maxNormalGoldSubmittedRating`: 可选，仅放宽普通金卡的最高 rating；特殊卡仍使用 `maxSubmittedRating`。84x10 默认普通金卡到 `89`，requirement special 仍到 `88`。
