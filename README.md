...
进入 EA FC Web App 后，等待页面、FSU 和 Enhancer 初始化。主面板标题显示 `Loop Runner v0.6.16`，日志出现 `Ready v0.6.16` 后即可开始；优化版 FSU 命中快速缓存时会进入 `trusted-provisional`，后台继续校验已恢复的 Club 缓存。Runner 会在每次保存 SBC 前只向 EA 校验本次选中的 Club 球员，全量校验结束后自动切换为普通 ready 状态。
