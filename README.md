...
- `DailyLoopRunner.user.js`: `0.2.81`
...
- `84x10 MVP (1 run)`
  ...
  - 若保护检查通过但 submit 仍不亮，会最多尝试 8 次用普通金 `<=88` 补分并重新保存检查；仍不亮则停止。
  ...
  - 自动 TOTW 前置 SBC 默认 `maxSubmittedRating: 85`、`blockSpecial: true`、`blockTradeable: true`，第一组 rare gold 材料允许 84-85。
