...
- `DailyLoopRunner.user.js`: `0.2.45`
...
84x10 测试推荐选择 `84x10 MVP (1 run)`。它会打开目标 SBC，让 FSU/Enhancer 填阵容，然后在提交前检查当前 squad。默认保护规则：只允许挑战要求的 1 张 84+ TOTW，不提交额外特殊卡，不提交可交易卡，不提交 89+。如果库存里没有符合保护规则的 84+ TOTW，live run 会先做 1 次 `84+ TOTW Upgrade` 并自动开包拿到 TOTW；如果 FSU 填满 11 人但放错特殊卡，live run 会优先保留 squad 里已有的合格 84+ TOTW，并把低于 84 的 TOTW、多余/违规特殊卡替换成符合保护规则的普通卡，再重新检查。`Dry run` 只会提示 live run 将执行前置 TOTW 或保存哪几处修复，不会提交 SBC。84x10 奖励包默认保留不打开。
如果 FSU 填了 11 人但提交按钮仍不可用，dry-run 会继续打印 squad inspection，并标记 live run 不会提交。
`10x 84+ Upgrade` 这类要求 1 张 TOTW/TOTS/FOF 的 SBC 会先确认可用 84+ TOTW，不足时先做 `84+ TOTW Upgrade`，再点击该 requirement 右侧的 `Add` 并由 FSU 补齐其它槽位；保护层只允许这 1 张 84+ TOTW，且仍受 `maxSubmittedRating` 和不可交易检查限制。
...
- `84x10 MVP (1 run)`
  ...
  - 若没有可提交的 84+ TOTW，会先提交 1 次 `84+ TOTW Upgrade`，自动打开 TOTW 奖励包并刷新库存。
  - 若 FSU 补全后特殊卡不符合要求，会生成 required TOTW repair plan：优先保留已有合格 84+ TOTW，替换低于 84 的 TOTW 或多余/违规特殊卡，再保存并重新检查。
  - 使用 FSU/Enhancer 填阵容，但提交前由脚本检查 squad。
  - 默认 `maxSubmittedRating: 88`、`requiredSpecialCount: 1`、`requiredSpecialKind: totw`、`requiredSpecialMinRating: 84`、`allowedSpecialCount: 1`、`blockTradeable: true`。
...
- 82+ 金卡保护只用于日常升级 loop。特殊 SBC，例如 84x10，有自己的提交前保护：默认只允许 1 张 84-88 的 TOTW，不允许额外特殊卡或可交易卡。
...
- `requiredSpecialKind`: 可选，目前支持 `totw`。用于要求允许的特殊卡必须是 TOTW。
- `requiredSpecialMinRating`: 可选，要求特殊卡的最低 rating；84x10 默认用 `84`，避免把低评分 TOTW 当成可提交材料。
- `autoTotwUpgrade`: 可选，对象或 `false`。当 `requiredSpecialKind: "totw"` 且库存没有可用 TOTW 时，live run 会先执行该 TOTW 前置 SBC；设为 `false` 可关闭自动补 TOTW。
