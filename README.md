...
- `DailyLoopRunner.user.js`: `0.2.25`
...
84x100 测试推荐选择 `84x100 MVP (1 run)`。它会打开目标 SBC，让 FSU/Enhancer 填阵容，然后在提交前检查当前 squad。默认保护规则：只允许挑战要求的 1 张特殊卡/TOTW/TOTS/FOF，不提交额外特殊卡，不提交可交易卡，不提交 89+。`Dry run` 会打印将要提交的卡和保护命中原因；live run 最多提交 1 次，奖励包默认保留不打开。
