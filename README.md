...
- `ratingSbcFill`: 可选，对象。启用通用评分 SBC 求解器；`priorityPiles` 默认可设为 `['unassigned', 'storage', 'transfer', 'club']`，只用于同一最低评分组合内的来源选择。`targetRating` 仅作为当前 Challenge 没有 `TEAM_RATING` 时的显式后备值，通常不应写死。
...
上述评分函数只由配置了 `ratingSbcFill` 的 `fillAndVerifySbc` 路径调用。目前影响范围是 `84+ TOTW Upgrade`、`84x10 MVP`、`84x10 Loop` 及 84x10 的自动 TOTW 前置；Daily、Provision、Rare Pack 和 Player Pick 仍走各自既有策略。
