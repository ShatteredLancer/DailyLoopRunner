...
- `DailyLoopRunner.user.js`: `0.4.29`
...
- `84x10 MVP (1 run)`
  ...
  - 若安全普通金补分已经耗尽且阵容仍达不到提交评分，live run 每个 84x10 最多自动完成 3 次 `2x 84+ Upgrade`。每次都先确认存在 6 张合格低分普通金稀有，开包入库成功后才重试原 84x10；材料不足或奖励包打不开时立即停止。
...
- `Provision Crafting Loop`
  ...
  - 开包并保留本轮低分重复材料后，只有当前 unassigned 中至少存在 1 张重复普金，才检查 `5 of 10 82+ Players Pick`。如果没有重复普金，会跳过前置 Pick 并直接继续原有 crafting 流程；仅有重复稀有金也不会触发前置 Pick。
  - 前置 Pick 尚未完成时，只补做未完成的 Challenge，领取并选择 Pick，然后继续原有 crafting 流程；如果已经完成则直接跳过。若触发检查时存在已领取但尚未选择的目标 Pick，会优先完成该 Pick。
  - 前置 Pick 复用独立 Pick loop 的普金比例、FSU/锁卡过滤、高分保护阈值、价格查询和自动/人工选择设置。本轮 Provision 包留下的低分重复卡会按 item id 保留，并在 Pick 选材时保护对应 definition，避免提前消耗原 crafting 材料；Pick 新结果也不会混入这些材料。
  - 非重复物品放入 club。
...
- `preCraftPlayerPickLoopId`: 可选，`provisionPackDualCrafting` 开包后的前置 Player Pick loop id。目标必须存在并且 `strategy` 为 `playerPickSbc`。
