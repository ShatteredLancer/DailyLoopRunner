...
`1 of 3 83+ Player Pick` 每次严格提交 3 张普通稀有金和 1 张普通普金；`4 of 10 82+ Player Pick` 每次完成两个 Challenge，两个阵都严格提交 9 张普通普金。两条 Pick loop 都强制保护 82+ 普通金卡，只会使用 81 及以下材料；配置层和提交前硬校验会同时拦截高分卡。默认各做 1 个 Pick；在 Options 中调整 `rounds` 可设定本次要做的 Pick 数量。
