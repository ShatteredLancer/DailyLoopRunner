...
Inventory-first SBC 会在选人阶段按 `definitionId` 去重，避免同一球员的两张重复卡被选入同一套 SBC 后保存时只剩 10/11 人；同时会继承 loop 级 `blockTradeable`。普通金卡的交易状态由 FSU `Only Untradeable` 决定，`blockTradeable` 继续保护可交易特殊卡和其它非普通金材料。
...
- 82+ 金卡保护用于所有非高分 SBC 的普通金/稀有金回收路径，包括 Daily Rare、Daily Rare Pack to 2x84+ 和 Provision Crafting。特殊 SBC，例如 84x10 和 84+ TOTW，有自己的评分与特殊卡提交规则。

## 4.1 当前 Daily 行为（0.4.2）

以下规则以当前脚本和 `DailyLoopRunner.loops.json` 为准；本文档中较早版本的行为描述如有冲突，以本节为准。

- 面板默认是简洁模式：loop 选择、`Start`、`Stop` 和最新日志。点击 `Options` 后展开运行选项、配置操作和完整日志；`L` 可收成可拖动的小图标。
...
`1 of 3 83+ Player Pick` 每次严格提交 3 张普通稀有金和 1 张普通普金；`1 of 5 83+ Player Pick` 每次严格提交 4 张普通稀有金；`4 of 10 82+ Player Pick` 每次完成两个 Challenge，两个阵都严格提交 9 张普通普金；`5 of 10 82+ Player Pick` 每次完成两个 Challenge，两个阵都严格提交 11 张普通普金。所有 Pick loop 都强制保护 82+ 普通金卡，只会使用 81 及以下材料；配置层和提交前硬校验会同时拦截高分卡。默认各做 1 个 Pick；在 Options 中调整 `rounds` 可设定本次要做的 Pick 数量。

Pick 结果按评分、特殊卡、非重复、FUT.GG 价格排序。最高评分档有两张或更多特殊卡时，或会影响选取边界的同类候选缺少价格时，Runner 会弹出手动选择窗口并等待确认。普通自动选择和手动选择完成后都会按既有 Unassigned 清理规则入 Club、Transfer List 或 SBC Storage。运行前应关闭其它插件的 Player Pick 自动选择，避免两个脚本同时确认同一个 Pick。
