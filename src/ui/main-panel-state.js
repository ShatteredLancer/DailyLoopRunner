...
import { rollingStorageRecoveryModeLabel } from '../config/runtime-options.js';
...
export function renderSelectionPolicySummary(options = {}) {
  ...
  const clubSpecialProtection = pickOptions.rollingProtectAllClubNonTotwSpecials === true
    ? 'protected'
    : 'fallback allowed';
  const clubCurrentPoolProvisions = pickOptions.rollingProtectAllClubNonTotwSpecials === true
    ? 'blocked by strict protection'
  ...
  const recoveryPileOrder = pickOptions.rollingRecoveryStorageFirst === true
    ? 'Storage first'
    : 'Unassigned first';
  const storageRecoveryPriority = rollingStorageRecoveryModeLabel(
