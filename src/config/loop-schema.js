...
function validateRollingStorageSink(value, path, errors) {
  ...
  if (value.status === 'resolved') {
    ...
    if (!isPlainObject(capability)) {
      ...
      const challengeRatings = Array.isArray(capability.challengeRatings)
        ? capability.challengeRatings.map(Number)
        : [];
      if (!challengeRatings.length
        || challengeRatings.some((rating) => !Number.isInteger(rating) || rating < 1 || rating > 99)
        || !challengeRatings.some((rating) => rating >= 87)) {
